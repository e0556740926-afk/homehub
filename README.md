...
- **קבלות שנסרקו נשמרות עם התמונה המקורית** (Supabase Storage) — לחיצה על קבלה בטאב "קבלות" פותחת אותה במסך מלא
- הצעת מתכונים אינטראקטיבית (4 שאלות הבהרה) + **תפריט שבועי מלא ל-7 ימים**, שניהם כוללים הערכת קלוריות למנה
- **חישוב שימוש חכם בבישול**: כשמתכון דורש "15 מ״ל שמן זית" אבל במלאי יש "1 יח׳" (בקבוק), המערכת לא מורידה את כל הבקבוק — היא עוקבת אחרי כמות השימוש המצטברת מול הערכת התכולה של האריזה (למשל בקבוק שמן זית ~750 מ״ל), ומוסיפה לרשימת הקניות רק כשבאמת נגמר
