...
- הצעת מתכונים אינטראקטיבית (4 שאלות הבהרה) מבוססת Gemini
- מסך בית עם ברכה, סטטיסטיקות, נוספו לאחרונה ומתכונים מועדפים
- **רשימת קניות שבועית אוטומטית**: ב-Netlify Function מתוזמנת (`weekly-list-scheduler.mjs`, רצה כל שעה) שבודקת אם הגיע היום/השעה שנבחרו בהגדרות (מסך ⚙️ בדף הבית), ואם כן — מוסיפה לרשימה כל מוצר שנקנה בעבר (מופיע ב-price_history) אך לא נמצא כרגע במלאי ולא כבר ברשימה. לוגיקה דטרמיניסטית מבוססת היסטוריית קניות אמיתית, לא ניחוש AI.

פתוח להמשך: מאגר מתכונים שמורים נוסף, התראות תפוגה חכמות, בית משותף (רב-משתמשים).
